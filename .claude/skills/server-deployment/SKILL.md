...
- **Scheduled commands are `ai:cost-alert` (08:00) and `jobs:run-alerts` (07:00)** — nothing else. Read `routes/console.php` before claiming anything else runs on cron. Production needs the `schedule:run` cron AND a queue worker (`resumegen-queue` systemd service) — the job-alert digest mailable is `ShouldQueue`.
