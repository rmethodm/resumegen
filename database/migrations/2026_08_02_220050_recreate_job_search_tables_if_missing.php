...
return new class extends Migration
{
    public function up(): void
    {
        ...
        // that resumes exists again with its new schema. Detecting whether the
        // constraint already exists isn't portable across drivers, so just
        // attempt it and swallow "already exists"/unsupported-on-sqlite errors.
        if ($jobSearchesExisted && Schema::hasColumn('job_searches', 'resume_id')) {
            try {
                Schema::table('job_searches', function (Blueprint $table): void {
                    $table->foreign('resume_id')->references('id')->on('resumes')->nullOnDelete();
                });
            } catch (Throwable) {
                // Already attached, or the driver doesn't support adding it here.
            }
        }
