...
class CoverLetterTest extends TestCase
{
    ...
    /**
     * Cover letters are unlimited now that billing is gone. A user sitting on a pile of
     * them must still be able to make another — a featureGate here means a paywall
     * has crept back in.
     */
    public function test_cover_letters_are_unlimited(): void
    {
        $user = User::factory()->create();
        CoverLetter::factory()->count(50)->create(['user_id' => $user->id]);
        ...
        $this->assertSame(51, $user->coverLetters()->count());
    }
}
