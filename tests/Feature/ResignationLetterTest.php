...
class ResignationLetterTest extends TestCase
{
    ...
    public function test_generate_returns_body_and_remaining(): void
    {
        ...
        $res = $this->actingAs($user)->postJson(route('resignation-letters.generate', $letter), [
            'last_day' => now()->addWeeks(2)->format('Y-m-d'),
            'tone' => 'formal',
        ]);

        $res->assertOk();
        $res->assertJsonStructure(['body', 'remaining']);
        $this->assertDatabaseHas('resignation_letters', [
            'id' => $letter->id,
            'body' => 'I am writing to inform you of my resignation.',
        ]);
        $this->assertDatabaseHas('ai_requests', [
            'user_id' => $user->id,
            'feature' => 'resignation_letter',
            'status' => 'success',
        ]);
    }

    public function test_generate_other_user_forbidden(): void
    {
        $owner = User::factory()->pro()->create();
        $other = User::factory()->pro()->create();
        $letter = ResignationLetter::factory()->for($owner)->create();

        $this->actingAs($other)->postJson(route('resignation-letters.generate', $letter), [
            'last_day' => now()->addWeeks(2)->format('Y-m-d'),
            'tone' => 'formal',
        ])->assertForbidden();
    }
    ...
    public function test_generate_quota_exhausted_returns_402(): void
    {
        ...
        $this->actingAs($user)->postJson(route('resignation-letters.generate', $letter), [
            'last_day' => now()->addWeeks(2)->format('Y-m-d'),
            'tone' => 'formal',
        ])->assertStatus(402);
    }
    ...
    public function test_generate_ai_service_failure_returns_503_and_leaves_body_unchanged(): void
    {
        $this->fakeServiceFailure();
        $user = User::factory()->pro()->create();
        $letter = ResignationLetter::factory()->for($user)->create(['body' => 'Original body']);
        ...
        $this->assertDatabaseHas('resignation_letters', ['id' => $letter->id, 'body' => 'Original body']);
    }
}
