...
- **Auth:** Laravel Fortify (session-based), Sanctum (API tokens) — swapped from Breeze on 2026-08-02. `app/Providers/FortifyServiceProvider.php` and `config/fortify.php` are the relevant files. `User` implements `MustVerifyEmail` — new registrations must verify before accessing the app. The `verified` middleware gates all main routes (`web.php` ~line 41 — check the current file rather than trusting this line number, it has drifted before). Resumegen's custom 2FA system (`TwoFactorController`, `TwoFactorChallengeController`, etc.) is separate from and unrelated to Fortify's own optional two-factor feature, which stays disabled — do not enable `Features::twoFactorAuthentication()` without asking first. Disabled accounts (`users.disabled_at`) are rejected at Fortify login and by `EnsureUserNotDisabled` on web requests.
...
- **Media:** none in use. The resume photo feature was removed; `Resume` no longer implements `HasMedia`. `spatie/laravel-medialibrary` and `smalot/pdfparser` (dead since the PDF import removal) were both uninstalled 2026-08-04 along with the published `config/media-library.php` and the orphaned `media` table (forward-only drop migration). If Boost's `medialibrary-development` skill or Media Library rules block resurfaces, that means Boost re-added the package — verify against `composer.json` before trusting it.
- **AI:** the 2026-07-21 removal (`AiService`, `AiRequest`, `ai_requests`, etc. — see "Removed Features" below) still stands, but a narrower AI slice shipped back on 2026-08-04 (`ecb012b`): `ResumeAiController` + `App\Services\OpenAiResumeAssistant` + `AiUsageLimiter`, gated by `config/ai.php` (`AI_ENABLED` + `OPENAI_API_KEY`, both unset by default — the feature ships disabled). It powers exactly two Optimize-tab actions, bullet rewrite and summary generation, both edit-in-place with no auto-save. See `docs/ai-reintroduction-map.md` for the design rationale — its "nothing here is live" framing is now stale for the Tier-1 slice; the rest of that doc (JD match, import, cover letters) is still unbuilt.
- **Billing:** none — see "Billing — there is none" below. No Cashier, no Stripe, no pricing instrumentation.
- **Job Imports:** `/jobs-imports` (`JobImportsController` + `App\Services\JobImport\{JobImportSearch,AdzunaClient,UsaJobsClient}`, `App\Models\ImportedJob`) is a real, tested feature shipped 2026-08-05 (`a68322f`, `520c778`) — live search against Adzuna/USAJOBS plus persistence to `imported_jobs`, config via `ADZUNA_APP_ID`/`ADZUNA_APP_KEY`/`USAJOBS_KEY`/`USAJOBS_EMAIL` in `.env.example` (a source with no key is skipped, never an error). Not the same as the removed 2026-07-21 Job Search (`/jobs`) — that stays gone; this is a separate, newer surface. Matching, gap analysis, tailoring, and cover letters stay frontend stubs per the controller's own docblock — do not build those without asking first.
...
### Authorization
**No policy classes exist** — `app/Policies/` is empty and `$this->authorize()` is never called anywhere in `app/` (verified 2026-08-07). Every mutation is gated by inline `abort_unless($user->id === $resume->user_id)` or equivalent, matching Resumo's own convention. This corrects an earlier version of this doc that described a `ResumePolicy` as already gating resume mutations — that class was never built. `docs/superpowers/plans/*` and `docs/superpowers/specs/*` reference `ResumePolicy`/`CoverLetterPolicy`/`JobApplicationPolicy` throughout — those are unbuilt design docs for features that were deferred or removed, not evidence the classes exist. If a policy layer gets built for real, update this section and grep those docs before trusting their authorization claims.

### Frontend page structure (post-2026-08-02 import)
`Pages/Resumes/Workstation.tsx` is the editor: a section-rail (`SectionPanel`) + stacked per-section forms (`inspector.tsx`/`inspector-sections.tsx`/`inspector-fields.tsx`), autosaved via `useAutosave` (debounced `router.put`, no beforeunload beacon). **`Components/resume/resume-preview.tsx` (the React equivalent of the PDF template renderer, all 24 templates) is wired in** — the Workstation's Review tab renders it live (`Workstation.tsx` ~line 750), and `Pages/Resumes/PublicShare.tsx` renders it too for the public share-link view. `Pages/Resumes/Compare.tsx` diffs two versions side by side. `Pages/Dashboard.tsx` lists resume groups with score, version tray, and the usual rename/duplicate/delete/compare actions.
...
- **Job applications tracker** (removed in `93c1c14`; `application_contacts` and `interview_notes` are dropped by migration and stay out of scope). **Came back on 2026-08-06 as the Job Application Kanban** (`/job-applications`): `App\Http\Controllers\JobApplicationController` + `App\Models\JobApplication`, columns Saved/Applied/Interviewing/Offer/Rejected, drag-and-drop via `@dnd-kit`. `job_applications` was kept live through the removal on the assumption `AnalyticsController` read it for a dashboard count — that controller does not exist anywhere in the current codebase (verified via repo-wide grep), so that dependency claim was stale by the time this note was corrected; nothing else depends on the table's shape besides the new tracker itself. `resume_id` (dropped in the 2026-08-02 reset) was re-added via a new migration, not restored from the old one.

Deleted on 2026-07-21 — code, routes, models, migrations, config and tests:

- **All AI.** `AiService`, `AiPrompts`, `AiRequest`, `AiUsageReport`, `OpenAiUsageService`, `ModerationException`, `AiDisabledException`, `EnsureAiEnabled` + the `ai_enabled` alias, `AiSuggestionController`, `InterviewCoachController`, `ResumeImportController`, `ai:cost-alert`, `ai:prune-flagged`, `config/openai.php`, the Filament `AiOverviewPage` / `AiUsersPage`, the `aiEnabled` Inertia prop, `users.ai_limit_override` / `ai_blocked` / `ai_usage_reset_at`, and the `ai_requests` table. The features that went with it: bullet coach, ATS keywords, interview coach, cover-letter AI draft, and resume PDF/LinkedIn import. Frontend: `useAiSuggestion`, `AtsMatchPanel`, `JdMatcher` (+ its test), `PdfImportModal`, `plainText.ts`. **Bullet rewrite and summary generation came back on 2026-08-04** as a narrower, unrelated slice — `config/ai.php` was recreated (not the file this removal deleted), see "AI" under Stack above; do not read this bullet as meaning those two features are gone today.
...
Survived that removal and must not be described as gone: the **JobRole / JobTitle / JobSkill taxonomy** with its seeders (still run by `DatabaseSeeder`), `AutocompleteController` and `SkillCategories` — only the admin CRUD over the taxonomy is gone, so **taxonomy is now seeder-managed with no UI**; **`StrengthScorePanel`** (server-side scorer, never AI); and the builder's `resumes.target_company` / `target_job_description` fields — the Optimize tab now holds a plain "Job description" textarea where the ATS panel used to be. (`target_title`, added alongside `target_company` in the same 2026-07-20 migration, was never adopted by anything in `app/` and was dropped 2026-08-04 — don't describe it as a live field.)
...
- **Portfolio.** `PortfolioController`, `PortfolioMessage` model + mail + factory, the public `/p/{slug}` page and settings page (`resources/js/Pages/Portfolio/Show.tsx`, `resources/js/Pages/Settings/Portfolio.tsx`), the Portfolio nav item, and `users.portfolio_slug` / `portfolio_headline` / `portfolio_bio` / `portfolio_is_public` / `portfolio_links`. (Correction, 2026-08-04: this bullet used to claim the `/messages` page and `MessagesController` — the `ResumeThread` share-link comment inbox — were "deliberately kept." They were not: `resume_threads` and `resume_thread_messages` were dropped by the same-day `2026_08_02_215300_drop_resumes_shares_and_messages_tables.php` migration during the relational-rewrite reset, and neither `MessagesController` nor a `ResumeThread` model exist in `app/` today. Don't resurrect this claim without checking `app/Http/Controllers/` first.)
...
2. **No template React components in the PDF path** — server renders PDF (`ResumeExport` + `resumes.export.pdf` Blade). A React preview component (`resume-preview.tsx`) exists and mirrors the same 24 templates; it's wired into the Workstation's Review tab and `PublicShare.tsx`, but PDF generation itself stays server-side and doesn't route through it.
...
8. **LLM use is narrow and opt-in** — strength score, autocomplete, and keyword overlap stay deterministic server-side code. The one exception is the Optimize tab's bullet rewrite / summary generation (see "AI" under Stack above), disabled by default via `AI_ENABLED`. Expanding AI beyond that Tier-1 slice is still a product decision, not an implementation detail; ask first.
