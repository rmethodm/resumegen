...
### Templates
Nine templates (`classic`, `modern`, `minimal`, `minimal-ruled`, `executive`, `ats`, `skills-first`, `academic`, `bold`) are stored as a string column on `resumes`. The live preview in `Edit.tsx` is an `<iframe>` that loads `GET /builder/{resume}/preview` (the server-rendered PDF stream) — there are no inline React template components in the editor. A cache-busting `?t=<timestamp>` query param on `pdfSrc` forces the iframe to reload after each save. The PDF Blade view (`resources/views/resume-pdf.blade.php`) is the single source of truth for all template rendering. The editor's template picker (a card grid in `Edit.tsx`) shows static preview images per template (see Resume thumbnails).

### Resume thumbnails
`GET /builder/{resume}/thumbnail` (`builder.thumbnail`, auth + ownership) serves a cached PNG of the resume's first PDF page, generated by `App\Services\ResumeThumbnailGenerator` (DomPDF → Imagick/Ghostscript, ~400px wide). Cache lives at `storage/app/thumbnails/{id}.png` (gitignored) and regenerates lazily only when `resume->updated_at` is newer than the file mtime; it's deleted in the `Resume` `deleting` observer. On any generation failure the route logs a warning and returns a GD-tinted placeholder (resume `accent_color`) so the UI never breaks. Shown per row in `ResumeBuilder/Index.tsx`. Template-picker previews are static committed assets at `public/images/templates/{template}.png`, rendered from `App\Data\SampleResume` by `php artisan thumbnails:templates` (`GenerateTemplateThumbnails`) — re-run at deploy / when a template's Blade changes. **Production requires the Imagick PHP extension + Ghostscript binary.**
...
**Admin growth analytics:** `GET /admin/growth` (`admin.growth.index`, master-admin gated) → `AdminGrowthController@index` → `Admin/Growth/Index.tsx`. Backed by `App\Services\GrowthReport` (existing tables only, no new tracking): `signupsSeries($period)` (new users/day), `funnel()` (Signed up → Activated[`User::has('resumes')`] → Paying[`plan_tier` in starter/pro/agency]), `activationRate()`/`conversionRate()` (percentages), `avgDaysToConvert()` (registration → first subscription, null when none), `referral()` (referred signups + how many converted, via `referred_by_user_id`). Page reuses `Admin/Ai/Charts.tsx` with a 7d/30d/all selector.

**Weekly retention cohorts:** `user_activity_days` (unique `(user_id, activity_date)`, no timestamps) records one row per user per active day, stamped by `TrackActivity` middleware (appended to the `web` group in `bootstrap/app.php`; session-gated to one `insertOrIgnore` per day; no-op for guests). `GrowthReport::retentionCohorts($weeks=6)` builds the cohort matrix (signup-week Monday → `{cohort, size, retention[]}`, `retention[k]` = % active in week k) from one users query + one activity query, computed in PHP. `AdminGrowthController` passes `retention`; `Admin/Growth/Index.tsx` renders an opacity-shaded cohort table. `MAX(activity_date)` doubles as a last-active signal. History accrues going forward (no backfill).

**MRR snapshots:** `revenue_snapshots` (one row per day, `captured_on` unique) stores `mrr_cents`, `paying_users`, `free_users`, `active_subscriptions`, `tier_counts` (json). Written by the `revenue:snapshot` command (`CaptureRevenueSnapshot`) — an idempotent `updateOrCreate` on `today()` using `RevenueReport`, scheduled daily at 23:55. `RevenueReport::mrrSeries($period)` reads it back as a `LineChart` series (`count` = MRR whole-dollars, `cost_cents` = exact). `AdminRevenueController` passes `mrrSeries`; `Admin/Revenue/Index.tsx` renders an "MRR over time" chart. History starts the day the command first runs (no backfill).

**Delivery log:** `system_events` is an append-only table (`created_at` only — `SystemEvent` uses `public const UPDATED_AT = null`). Columns: `channel` (`mail`|`stripe_webhook`), `type` (mail subject or Stripe event type), `status`, `recipient`, `meta` (json). Written by best-effort `SystemEvent::record(...)` from two `Event::listen` closures in `AppServiceProvider::boot()` — `MessageSent` (outbound mail) and Cashier's `WebhookReceived` (inbound Stripe). Both swallow their own exceptions so logging never breaks a send/webhook. Pruned after 30 days by `system-events:prune` (scheduled daily). Surfaced as the "Recent deliveries" section on the Ops dashboard (`recentEvents` prop, latest 50) — no separate route.

**Admin ops dashboard:** `GET /admin/ops` (`admin.ops.index`, master-admin gated) → `AdminOpsController@index` → `Admin/Ops/Index.tsx`. Surfaces queue depth (`jobs`/`failed_jobs` counts), a failed-jobs table parsed from `failed_jobs.payload`, the scheduled-task inventory (`app(Schedule::class)->events()`), and config-health checks (queue/mail driver, Stripe secret + webhook secret, OpenAI key). Actions: `POST /ops/failed/{uuid}/retry` (`admin.ops.retry`, `Artisan::call('queue:retry')`) and `DELETE /ops/failed/{uuid}` (`admin.ops.forget`) — both audited via `AdminAuditLog::record()` with the uuid in `meta`/`description` (failed jobs aren't Eloquent models, so `target` is null). Webhook/mail *delivery history* is intentionally not shown — nothing persists it; the dashboard reports config status instead.

**Admin revenue reporting:** `GET /admin/revenue` (`admin.revenue.index`, master-admin gated) → `AdminRevenueController@index` → `Admin/Revenue/Index.tsx`. Numbers come from the `App\Services\RevenueReport` service: `tierCounts()` (users grouped by `COALESCE(plan_tier,'free')`, zero-filled), `mrrCents()` (= Σ tierCount × `config('services.stripe.tier_prices')[tier]` over starter/pro/agency — a monthly-headline estimate, not exact ARR), `activeSubscriptions()` (Cashier `subscriptions` where status active/trialing and not ended), `newSubscriptionsSeries($period)` (daily counts; `all`=90d cap), `recentSubscriptions()`. `liveActiveSubscriptions()` is an optional fail-soft Stripe reconcile (cached 1h, null when `cashier.secret` empty) — same pattern as `OpenAiUsageService`. Page reuses `Admin/Ai/Charts.tsx` (`Stat`/`BarList`/`LineChart`/`fmtCents`) with a 7d/30d/all selector. Tier prices configured in `config/services.php` → `stripe.tier_prices` (env `PRICE_{STARTER,PRO,AGENCY}_CENTS`, defaults 900/1900/4900). True historical MRR time-series is out of scope (no snapshot table).

**Admin content management:** `AdminContentController` (master-admin gated) gives one tabbed moderation view over all user content. `GET /admin/content` (`admin.content.index`) → `Admin/Content/Index.tsx` — `?type=resumes|cover-letters|jobs|portfolios` (default resumes), `?q=` searches the item title + owner name/email, 25/page, with per-type `counts` for the tab badges. `GET /admin/content/resumes/{resume}` (`admin.content.resume.show`) → `Admin/Content/Resume.tsx` renders a read-only view of a resume's sections (no edit, no PDF — owner-policy-gated). Destructive actions (all audited via `AdminAuditLog::record()`): `DELETE` resume/cover-letter/job (`content.{resume,cover-letter,job}.destroy`), `PATCH content/share-links/{shareLink}/disable` (`content.share-link.disable`, sets `is_active=false`), `PATCH content/users/{user}/unpublish-portfolio` (`content.portfolio.unpublish`, sets `portfolio_is_public=false`). View+delete+unpublish only — no content editing.

**Admin audit log:** `admin_audit_logs` is an append-only table (`created_at` only — `AdminAuditLog` uses `public const UPDATED_AT = null`). Columns: `admin_user_id` (FK→users, nullOnDelete), `action` (dot-namespaced, e.g. `user.delete`), `target_type`/`target_id` (nullable morph), `description`, `meta` (json), `ip_address`. Record privileged actions with the static best-effort `AdminAuditLog::record(string $action, ?Model $target, string $description, array $meta = [])` — it reads `auth()->id()` + `request()->ip()` and swallows its own exceptions so it never breaks the action it logs. Already wired into `AdminUserController` (toggle-pro/agency, delete) and `AdminAiController` (reset-quota, set-limit, block, flagged-delete). `GET /admin/audit` (`admin.audit.index`) → `AdminAuditController@index` → `Admin/Audit/Index.tsx` — paginated feed filterable by `?action=` and `?admin=`. **When adding any new privileged admin write action, call `AdminAuditLog::record()` on its success path.**
