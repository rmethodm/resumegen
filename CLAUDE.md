...
- **Auth:** Laravel Fortify (session-based; replaced Breeze in the 2026-08-02 foundation swap), Sanctum (API tokens). `User` implements `MustVerifyEmail` — new registrations must verify before accessing the app. The main authenticated group in `web.php` runs `['auth', 'verified']`; `RequiresTwoFactorChallenge` is appended globally to the `web` group in `bootstrap/app.php` (the old `two_factor_challenge` route alias was removed 2026-09-24 as redundant). `RequireTwoFactorChallengeOnLogin` (Login event listener) sets the pending-challenge flag on every login of a 2FA user, including remember-cookie restores. OAuth login (Google/GitHub/Microsoft via `laravel/socialite` + `socialiteproviders/microsoft`, added 2026-09-06) is additive on top of this: `App\Http\Controllers\Auth\SocialiteController` (`routes/auth.php`) auto-links to an existing account only when the provider confirms the email is verified, otherwise the user is sent back to password login; new OAuth signups share `App\Actions\Fortify\RegistrationIpLimiter` with password registration.
- **PDF:** `barryvdh/laravel-dompdf` — server-side generation. Current routes: `GET /resumes/{resume}/export` (download), `GET /resumes/{resume}/preview` (inline stream). All five DomPDF call sites (web, mobile API, extension API, public share) go through `App\Support\PdfExport::for($resume)->stream()|->download()`. The legacy `builder/{resume}/pdf|preview|html-preview` routes redirect to those.
...
- **AI:** `openai-php/laravel` — credit ledger + Cashier gates; Workstation Rewrite/Generate HTTP is unrouted for v1, but `resumes.ai-review` and the Q&A-bank drafts are live and metered. See "AI — subscription credit gates" below.
...
**Single-shell rework (2026-09-18, uncommitted):** the old 4-mode layout switcher (`users.workstation_layout`) was removed — migration `2026_09_18_074502_drop_workstation_layout_from_users_table` — and `WorkstationLayouts.php` deleted. The Workstation is now one shadcn `Card` shell with exactly two tabs, `WORKSTATION_TABS = ['Edit', 'Optimize']` (`workstation-format-toolbar.tsx`); Notes/checkpoints moved out of the main layout into a slide-over `Components/workstation/notes-sheet.tsx` (`NotesSheet`, wraps `NotesPanel` + `SnapshotsPanel`). A parallel shadcn sidebar/theme exploration (`resources/js/shadcn-demo/**`, `docs/mockups/*.html`) is prototype work, not wired into the real app.

**Legacy builder endpoints:** the five that 500'd (`builder.store`, `builder.docx`, `builder.thumbnail`, `builder.duplicate`, `builder.create-variant` — they referenced deleted services/columns) were deleted on 2026-08-19, routes and methods both. Working equivalents live on `ResumeController` (`resumes.download-docx`, `resumes.duplicate`). On 2026-09-24 the legacy save/delete routes (`builder.update`, `builder.beacon`, `builder.destroy`) were deleted too (they validated the pre-relational JSON schema and bypassed the base-version delete guard), along with `resume-pdf.blade.php` and `App\Data\ResumeRules`. The surviving `builder.*` routes are redirects (`builder.edit` → Workstation, `builder.pdf` → `resumes.download`, `builder.preview/html-preview` → `resumes.preview`) plus `builder.share-url`. The legacy `ShareLinkController` (`share.store`/`share.update`) stays — `Shares/Index.tsx` calls it.
...
`HandleInertiaRequests::share()` passes `auth.user` — an explicit `{id, name, email, email_verified_at}` shape, never the model (the model once leaked the decrypted TOTP secret; sensitive columns are also in `User`'s hidden list) — `flash.{success,error}`, and for authenticated users `aiCredits: { balance, subscribed, canPurchase }` (null for guests). No `aiEnabled`, `impersonating`, or `featureGate` prop — AI gating uses the credit ledger + Cashier `subscribed('default')`, not a generic feature-gate bag.
...
**Model:** only active Cashier subscribers may hold/buy/spend AI credits. Generative routes **reserve** credits before the model call via `AiUsageLimiter::reserve()` (transaction + `lockForUpdate` on the user row, re-checks the gates, inserts the debit), attach `ai_request_id` on success, and append a compensating `refund` row on failure — so concurrent requests cannot overdraw. The `ai_credit_ledger` is append-only (balance = sum of entries; no expiry while subscribed). Upstream model failures return **502**. Starter credits are granted only when the subscription webhook status is `active`/`trialing` (`created` or `updated`). Gates: `subscribed('default')` + balance ≥ cost + not `users.ai_blocked`. HTTP: **402** = not subscribed or insufficient credits; **429** = `ai_blocked`. Past-due is treated as not subscribed for AI. Credits are per user, not per resume.

**Live metered AI routes:** `resumes.ai-review` (`ResumeController::aiReview`, throttle 20/min), `qa-bank-entries.draft` (throttle 20/min), and the extension `POST qa-bank/draft`. Rewrite (bullet/summary), Optimize Generate, and section rewrite are unrouted. `AiService::rewriteSection` remains an orphan — do not re-expose without metering. Optimize diagnose (keyword overlap) stays free.
...
## Job search / imports — removed, then partly back

**`/jobs-imports`, board clients, alerts, and URL import are gone** (2026-08-26). Dropped tables: `job_searches`, `imported_jobs`, `scraped_jobs`. **`job_listings` was recreated 2026-09-17** for the Browse Jobs pool: `JobListingController` (`/jobs/browse`, case-insensitive `whereLike` search with Postgres-only `pg_trgm` GIN indexes), `JobPoolController` (`/jobs/pool`), fed by the `ImportJobListings` / `ImportCompanies` commands. `ScrapeJobsCommand` was deleted 2026-09-24 (referenced a removed class).
...
**Partly back (2026-09-14):** a minimal admin surface exists again — `routes/admin.php` with `/admin/schedule` (`Admin\ScheduleController`, edits `scheduled_task_configs`), the `admin` alias → `EnsureUserIsAdmin`, and `users.is_admin` (migration `2026_09_14_150000_add_is_admin…`). Admin Ziggy routes are published only to admins (`config/ziggy.php` `admin` group, rendered in `app.blade.php` when `is_admin`). The rest of the old admin stays gone.

`users.disabled_at` and `EnsureUserNotDisabled` survive (auth and the API refuse disabled accounts — the API gets a JSON 403 via `user.not_disabled` on both token groups), but nothing in the app sets the column any more — set it directly in the DB if needed.

**App backups:** `spatie/laravel-backup` v10 (`config/backup.php`, disk `backups` → `storage/app/private/spatie-backups`). `routes/console.php` always schedules `backup:clean` 01:00, `backup:run` 01:30, `backup:monitor` 01:45 as code defaults; `scheduled_task_configs` rows can only override those three commands' cron/enabled flag, and a DB failure at boot logs a warning and falls back to the defaults (before 2026-09-24 an empty table silently scheduled no backups). CLI/schedule only.

## API Layer

Token-based Sanctum API at `/api`. `config/sanctum.php` sets `'guard' => []` (intentionally empty) — only token-auth works, no session fallback. Two client surfaces, distinguished by token ability — enforced by Sanctum `abilities:extension` / `abilities:mobile` route middleware (so a wrong-ability token gets 403 before validation), with the in-app `ensureExtensionToken`/`ensureMobileToken` checks kept as defence in depth. `/api/*` always renders JSON errors (`shouldRenderJsonWhen` in `bootstrap/app.php`). Extension tokens are pruned to the newest 5 per user; there is no global Sanctum expiry.

- **Extension** (`/api/extension/*`, ability `extension`): fill-profile payloads for the Resumegen Apply browser extension. Tokens issued from the Profile page (paste-token) or, as of 2026-09-14 Phase C, minted server-side by `GET/POST /extension/connect(/token)` (session-auth, `password.confirm`-gated) and handed to the extension via `chrome.runtime.sendMessage`/`externally_connectable` — needs `RESUMEGEN_EXTENSION_ID` set (`config('services.resumegen_extension.id')`) to fire; paste-token stays as the fallback. Read endpoints: `me`, `resumes`, `resumes/{id}/fill-profile`, `qa-bank`, `qa-bank/match`, `resumes/{id}/pdf` (DomPDF stream, mirrors `Api\ResumeController::pdf`). Write endpoints (all 2026-09-14 extension-upgrade spec phases): `POST qa-bank` (save entry), `POST qa-bank/draft` (match-first against `QaBankMatcher`, AI fallback via the same `AiUsageLimiter`/`AiCreditService` gate as `QaBankEntryController::draft` — 402/429 conventions apply here too), `POST job-applications` (reuses `StoreJobApplicationRequest`, logs a `job_application_status_events` row same as the Kanban), `PATCH resumes/{id}/target-job-description` (JD import via the extension's right-click context menu; `max:10000`, `UpdateResumeTargetJobDescriptionRequest`).
