...
1. **Identify the baseline.** One of:
   - A prior commit: find it with `git log --oneline -- resources/css/app.css` (or the date the user gave, e.g. "2 days ago").
...
3. **Revert the whole kind, not just the named file.** If one CSS file is restored to a baseline, restore every theme file of the same kind (all theme CSS, fonts in `app.blade.php`, and any hardcoded accent shadows) to that same baseline so they stay consistent. State which files you reverted.
4. **Rebuild and verify.** Run the asset build (`npm run build` / dev server) and confirm the UI actually shows the baseline palette.
5. **Keep StyleSeed / ui-craft token docs in sync** with the restored palette when those files exist (`STYLESEED.md`, `.ui-craft/tokens.md`).
