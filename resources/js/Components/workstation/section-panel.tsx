...
/** Band score below this is treated as weak in the grayscale strip. */
const WEAK_BAND_SCORE = 20;

function useIsNarrow(query = '(max-width: 639px)'): boolean {
...
export function SectionPanel({
    ...
    const nextGap = checklist.find((item) => !item.done) ?? null;
    const weakestBand = useMemo(() => {
        if (analysis.breakdown.length === 0) {
            return null;
        }

        return analysis.breakdown.reduce((lowest, band) =>
            band.score < lowest.score ? band : lowest,
        );
    }, [analysis.breakdown]);
    const hasWeakBand =
        weakestBand !== null && weakestBand.score < WEAK_BAND_SCORE;
    ...
    function toggleMobileChrome() {
        setMobileOpen((open) => {
            const next = !open;
            if (next && (hasWeakBand || nextGap !== null)) {
                setExpanded(true);
            }
            ...
            >
                <div className="flex flex-wrap items-center gap-x-5 gap-y-3">
                    ...
                    >
                        ...
                        <span className="ml-1 hidden text-xs font-medium text-ink-muted sm:inline">
                            Resume score
                        </span>
                    </div>

                    {showChrome && analysis.breakdown.length > 0 && (
                        <div className="flex min-w-0 flex-1 flex-wrap items-center gap-1.5">
                            {analysis.breakdown.map((band) => {
                                const isWeakest =
                                    hasWeakBand &&
                                    weakestBand?.label === band.label;
                                ...
                                return (
                                    <button
                                        ...
                                        title={
                                            isWeakest
                                                ? `${band.label} needs work — jump to next step`
                                                : `Fix ${band.label} — jump to next step`
                                        }
                                        className={cn(
                                    ...
                                    >
                                        <div
                                            className={cn(
                                                'flex justify-between text-[11px] font-semibold',
                                                isWeakest
                                                    ? 'text-ink'
                                                    : 'text-ink-muted',
                                            )}
                    ...
                    <Button
                        ...
                        variant="ghost"
                        size="sm"
                        aria-expanded={isNarrow ? mobileOpen : expanded}
                        aria-controls={
                            isNarrow
                                ? 'score-strip-chrome'
                                : 'score-strip-details'
                ...
                {showChrome && (
                    <div id="score-strip-chrome">
                        {!expanded && nextGap && (
                            <button
                                type="button"
                                onClick={() => onJumpChecklist(nextGap)}
                                className="focus-ring mt-2 flex w-full items-start gap-2 rounded-md border border-dashed border-ink/25 bg-surface px-2.5 py-2 text-left text-xs text-ink hover:border-ink/40"
                            >
                                <ExclamationTriangleIcon
                            ...
                            >
                                ...
                                {addable.length > 0 && (
                                    <div className="mt-4 border-t border-surface-border pt-4 md:col-span-2 md:mt-0 md:border-t-0 md:pt-0 xl:col-span-1 xl:border-l xl:border-t-0 xl:pl-6 xl:pt-0">
                                        <p className="mb-2 px-1 text-xs font-semibold text-ink-muted">
                                            Hidden sections
                                        </p>
