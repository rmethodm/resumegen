    ...
    return (
        <div className="relative shrink-0">
            <button
                ...
                className={`inline-flex h-7 w-7 items-center justify-center rounded-full text-gray-500 transition hover:bg-blue-50 focus-visible:opacity-100 group-hover:opacity-100 ${
            ...
            {open && (
                <>
                    ...
                    <div className="absolute right-0 top-8 z-50 w-40 rounded-lg border border-gray-200 bg-white py-1 shadow-lg">
                        <Link href={route('builder.edit', id)} className="block px-4 py-2 text-sm text-gray-700 transition hover:bg-gray-50">
                            Edit
                        </Link>
                        <button onClick={duplicate} className="block w-full px-4 py-2 text-left text-sm text-gray-700 transition hover:bg-gray-50">
                            Duplicate
                        </button>
                        <button onClick={destroy} className="block w-full px-4 py-2 text-left text-sm text-red-600 transition hover:bg-gray-50">
                            Delete
                        </button>
                    </div>
                </>
...
function ResumeGridCard({ card }: { card: ResumeCard }) {
    return (
        <div className="group flex flex-col overflow-hidden rounded-xl border border-gray-200 bg-white shadow-[0_1px_3px_rgba(79,70,229,0.05)] transition hover:-translate-y-1 hover:shadow-[0_2px_8px_rgba(79,70,229,0.1)]">
            {/* Decorative preview */}
            <Link href={route('builder.edit', card.id)} className="block h-40 border-b border-gray-200 bg-gray-50 p-5">
            ...
            <div className="flex flex-1 flex-col gap-2 p-4">
                <div className="flex items-start justify-between gap-2">
                    <Link href={route('builder.edit', card.id)} className="font-bold leading-snug text-gray-900 hover:text-blue-600">
                        {card.name}
                    </Link>
                ...
                <div className="flex items-center justify-between gap-2">
                    <span className="text-xs text-gray-400">Edited {editedAgo(card.updated_at)}</span>
            ...
            <div className="py-8">
                <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="mb-6">
                        ...
                        <p className="mt-1 text-sm text-gray-400">Your resume activity at a glance</p>
                    </div>
                        ...
                        {[
                            ...
                            const inner = (
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p className="text-lg font-bold leading-none tracking-tight text-gray-900">{value.toLocaleString()}</p>
                                        <p className="mt-0.5 text-[10px] font-semibold uppercase tracking-wide text-gray-400">{label}</p>
                                    </div>
                                </div>
                            );
                            return href ? (
                                <Link key={label} href={href} className="rounded-lg border border-gray-200 bg-white px-4 py-3 shadow-[0_1px_3px_rgba(79,70,229,0.05)] transition hover:border-blue-300 hover:shadow-[0_2px_8px_rgba(79,70,229,0.1)]">
                                    {inner}
                                </Link>
                            ) : (
                                <div key={label} className="rounded-lg border border-gray-200 bg-white px-4 py-3 shadow-[0_1px_3px_rgba(79,70,229,0.05)]">
                                    {inner}
                                </div>
                            );
                        })}
                    ...
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                        <Link
                            ...
                            className="flex min-h-[280px] flex-col items-center justify-center gap-3 rounded-xl border-2 border-dashed border-gray-300 text-gray-400 transition hover:border-blue-500 hover:text-blue-500"
                        >
                            <span className="flex h-12 w-12 items-center justify-center rounded-full bg-blue-50 text-2xl font-light text-blue-500">+</span>
