...
export default function JdMatcher({ content, initialJd }: Props) {
    ...
    const scoreColor = score <= 40 ? 'text-danger-text' : score <= 70 ? 'text-warning-text' : 'text-success-text';
    const barColor = score <= 40 ? 'bg-danger' : score <= 70 ? 'bg-warning' : 'bg-success';
                ...
                <div className="mt-4 space-y-4">
                    ...
                    <div>
                        <div className="mb-1 flex items-baseline justify-between">
                            <span className="text-xs font-bold uppercase tracking-wide text-ink-faint">Keyword coverage</span>
                            <span className={`text-sm font-bold tabular-nums ${scoreColor}`}>{score}%</span>
                        </div>
                        <div className="h-2 overflow-hidden rounded-full bg-surface">
                            <div className={`h-full rounded-full transition-all motion-reduce:transition-none ${barColor}`} style={{ width: `${score}%` }} />
                        </div>
                        <p className="mt-1 text-xs tabular-nums text-ink-faint">{matched.length} of {matched.length + missing.length} top keywords found in your resume.</p>
                    </div>

                    {/* Missing — the actionable gap */}
                    {missing.length > 0 && (
                        <div>
                            <p className="mb-1.5 text-xs font-bold uppercase tracking-wide text-warning-text">Missing keywords</p>
                    ...
                    {matched.length > 0 && (
                        <div>
                            <p className="mb-1.5 text-xs font-bold uppercase tracking-wide text-green-700">Already covered</p>
                            <div className="flex flex-wrap gap-1.5">
                                {matched.map(k => (
