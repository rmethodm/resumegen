...
class ResumeStrengthScorer
{
    public static function score(Resume $resume): array
    {
        ...
        if (! $hasSummary) {
            $tips[] = ['key' => 'summary', 'pts' => 15, 'order' => $order++, 'tip' => 'Add a professional summary'];
        }
        ...
        if (! $hasContact) {
            $tips[] = ['key' => 'contact', 'pts' => 15, 'order' => $order++, 'tip' => 'Complete your contact information'];
        }
        ...
        if (! $hasExp) {
            $tips[] = ['key' => 'experience', 'pts' => 15, 'order' => $order++, 'tip' => 'Add at least one work experience'];
        }
        ...
        if (! $hasEdu) {
            $tips[] = ['key' => 'education', 'pts' => 10, 'order' => $order++, 'tip' => 'Add your education'];
        }
        ...
        if (! $hasSkills) {
            $tips[] = ['key' => 'skills', 'pts' => 10, 'order' => $order++, 'tip' => 'Add at least 3 skills'];
        }
        ...
        if (! $hasMetric) {
            $tips[] = ['key' => 'metric', 'pts' => 10, 'order' => $order++, 'tip' => 'Add numbers or metrics to your bullets'];
        }
        ...
        if (! $hasMultiExp) {
            $tips[] = ['key' => 'experience', 'pts' => 10, 'order' => $order++, 'tip' => 'Add a second work experience'];
        }
        ...
        if (! $hasLinkedIn) {
            $tips[] = ['key' => 'linkedin', 'pts' => 5, 'order' => $order++, 'tip' => 'Add your LinkedIn URL'];
        }
        ...
        if (! $hasRichBullets) {
            $tips[] = ['key' => 'rich_bullets', 'pts' => 5, 'order' => $order++, 'tip' => 'Add 3+ bullets to a work experience entry'];
        }
        ...
        if (! $hasExtra) {
            $tips[] = ['key' => 'extra', 'pts' => 5, 'order' => $order++, 'tip' => 'Add a certification or custom section'];
        }

        usort($tips, fn ($a, $b) => $b['pts'] !== $a['pts'] ? $b['pts'] - $a['pts'] : $a['order'] - $b['order']);
        $tip = $tips[0]['tip'] ?? 'Your resume looks great!';
        $tipKey = $tips[0]['key'] ?? null;

        return ['score' => $points, 'tip' => $tip, 'tipKey' => $tipKey, 'checklist' => $checklist];
    }
}
